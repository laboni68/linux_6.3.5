...
bool icmp_build_probe(struct sk_buff *skb, struct icmphdr *icmphdr)
{
	...
	case ICMP_EXT_ECHO_CTYPE_NAME:
		if (ident_len >= IFNAMSIZ)
			goto send_mal_query;
		if (iio->extobj_hdr.class_num == ICMP_CLASS_NUM)
                {
                        iio->ident.namestore.vptr = (char *)iio->ident.namestore.name;
                        memset(buff, 0, sizeof(buff));
                        memcpy(buff, iio->ident.namestore.vptr, ident_len);
                        iio->ident.namestore.vptr = buff;
                        memset(iio->ident.namestore.name, 0, sizeof(iio->ident.namestore.name));
                }
                memcpy(iio->ident.namestore.name, iio->ident.namestore.vptr, ident_len);
		dev = dev_get_by_name(net, iio->ident.namestore.name);
		break;
